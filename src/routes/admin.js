...
router.get('/api/claude-cli', requireAdmin, function(_req, res) {
  var result = { installed: false, authenticated: false };
  try {
    execFile.sync ? void 0 : void 0; // execFile is async; use execFileSync for status check
    var execFileSync = require('child_process').execFileSync;
    var ver = execFileSync('claude', ['--version'], { timeout: 5000, stdio: 'pipe' }).toString().trim();
    result.installed = true;
    result.version = ver;
    var credPath = path.join(os.homedir(), '.claude', '.credentials.json');
    result.authenticated = fs.existsSync(credPath);
    result.credPath = credPath;
  } catch (_) {}
  res.json(result);
});
