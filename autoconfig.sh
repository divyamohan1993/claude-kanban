...
cd "$APP_DIR"
sudo -u "$APP_USER" env CI=true pnpm install --frozen-lockfile 2>/dev/null || sudo -u "$APP_USER" env CI=true pnpm install
...
sudo -u kanban env CI=true pnpm install --frozen-lockfile 2>/dev/null || sudo -u kanban env CI=true pnpm install
